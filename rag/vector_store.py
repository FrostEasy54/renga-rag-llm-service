...
# Multilingual MiniLM handles Russian well and is small enough for an RTX 3050.
...
class VectorStore:
    """
    Thin wrapper around ChromaDB for storing and retrieving construction norm chunks.

    Embeddings are generated locally via sentence-transformers (no API calls needed).
    The database is persisted to disk so it survives server restarts — you only need
    to call `index_documents()` once per knowledge base update.

    Typical usage:
        store = VectorStore(persist_dir=Path("chroma_db"))
        store.index_documents(Path("knowledge_base"))   # one-time setup
        results = store.query("нормы для жилых многоэтажных зданий", n_results=4)
    """
    ...
    def index_documents(self, knowledge_base_dir: Path) -> int:
        """
        Load all .txt files from `knowledge_base_dir` and upsert their chunks
        into ChromaDB.  Safe to call multiple times — existing chunks are
        overwritten by their deterministic ID, so you won't get duplicates.
    ...
    def _prepare_batch(
        ...
        for chunk in chunks:
            # Deterministic ID: source filename + chunk position
